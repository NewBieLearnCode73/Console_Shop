...
export class PaymentController {
  ...
  async getDigitalPaymentLink(
    ...
    return await this.paymentService.getDigitalPaymentLink(req.user.id, orderId);
  }

  @EventPattern('create_momo_payment')
  async createMomoPayment(@Payload() payload: { orderId: string; amount: number }) {
    await this.paymentService.createMomoPayment(payload.orderId, payload.amount);
  }
